...
{
    std::cerr << "Starting TCP IO Server" << endl; 
    // Start the server thread
    while (true)
    {
        //Client client = acceptClient(0);
    }
}
...
Client TcpServer::acceptClient(uint timeout)
{
   ...
   //TCPIOConnection ioConnection = new TCPIOConnection(file_descriptor, m_clientAddress);
   //ioConnection.Start();
...
void TCPIOServer::Construct(int port, int clientsQueueSize, Listener &serverResponseHandler)
{
   ...
   m_pserverResponseHandler = &serverResponseHandler;
   m_pserverResponseHandler->Notify(std::shared_ptr<Msg>(new StatusMsg(MsgIdServerConstructResponse, errorCode)));
}
